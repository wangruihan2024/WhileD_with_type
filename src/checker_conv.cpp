...
VarType check_binop(struct Expr *e, struct VarTypeEnv *env)
{
    ...
    VarType left_type = checkexpr(left, env);
    VarType right_type = checkexpr(right, env);
    switch (op)
    {
    case T_PLUS:
    case T_MINUS:
    ...
    case T_MOD:
        ...
        else // 改过后默认的T_BASIC枚举类型的顺序即满足优先级
        {
            ...
            {
                left = implicit_typeconv(right_type, left); // 修改挂在原AST上的左表达式
                return right_type;
            }
            else if (left_type.tbasic > right_type.tbasic)
            {
                right = implicit_typeconv(left_type, right);
                return left_type;
            }
            else
            {
    ...
    case T_OR:
        ...
        else // 都要默认转为int / 'bool'类 / 并且不用提醒，语义上是非零为真
        {
            if (left_type.tbasic != T_INT)
                left = implicit_typeconv(new_VarType_BASIC(T_INT), left); // 左侧隐式转换
            if (right_type.tbasic != T_INT)
                right = implicit_typeconv(new_VarType_BASIC(T_INT), right); // 左侧隐式转换
    ...
    else
    {
        ...
        case T_NOT:
            if (expr_type.tbasic != T_INT)
                right = implicit_typeconv(new_VarType_BASIC(T_INT), right);
